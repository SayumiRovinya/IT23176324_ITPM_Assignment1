...
- Prefer Playwright Test API (test, expect).
- Use fixtures for setup/teardown.
- Use page object pattern in pages/ to keep tests readable.
- Add meaningful test names and selectors to improve stability.

---

## CI (example)

- Run `npm ci`, `npx playwright install --with-deps`, then `npx playwright test`.
- Export artifacts (reports, videos, traces) for flaky test investigation.

---

## Troubleshooting

- If browsers fail to launch: run `npx playwright install --with-deps` (on CI or headless servers).
- If tests are flaky: increase timeouts in playwright.config.js or add explicit waits for stable selectors.
...
- Follow the existing test pattern and include clear commit messages.

---

## License

Specify project license in LICENSE file (if applicable).
